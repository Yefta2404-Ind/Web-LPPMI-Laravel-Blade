...
<style>
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* ── Layout ── */
    .create-container { max-width:1200px; margin:0 auto; padding:24px; }
    @media(max-width:768px){ .create-container{ padding:16px; } }
    ...
    /* ── Thumbnail Preview ── */
    .thumbnail-preview { margin-top:16px; display:none; }
    ...
    /* ── Gallery Preview ── */
    .gallery-preview-wrap { margin-top:16px; display:none; }
    .gallery-grid {
        display:grid;
...
<div class="create-container">
    ...
    <div class="page-header">
        ...
        <p class="page-description">Isi formulir di bawah untuk membuat berita baru yang akan menunggu persetujuan admin.</p>
    </div>
    ...
    <div class="form-card">
        <form id="newsForm" action="{{ route('staff.news.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-content">
                ...
                {{-- Upload Gambar (images[]) — gambar pertama otomatis jadi thumbnail --}}
                    ...
                    <div class="file-upload">
                        <div class="file-input" id="dropZone">
                            ...
                            <div class="upload-content">
                                ...
                                <div class="upload-hint">Pilih satu atau beberapa gambar sekaligus · JPG, PNG, GIF, WebP · Maks 2MB/gambar</div>
                            </div>
                        </div>
                    </div>

                    {{-- Thumbnail preview (gambar pertama) --}}
                    <div class="thumbnail-preview" id="thumbnailWrap">
                        <p style="font-size:12px;color:var(--gray-500);margin-bottom:8px;font-weight:500;">
                            📌 Thumbnail (gambar pertama)
                        </p>
                        <div class="preview-container">
                            <img id="thumbnailPreview" class="preview-image" alt="Thumbnail">
                        </div>
                    </div>

                    {{-- Gallery preview --}}
                    <div class="gallery-preview-wrap" id="galleryWrap">
                        <p style="font-size:12px;color:var(--gray-500);margin-bottom:8px;font-weight:500;">
                            🗂️ Semua Gambar
                        </p>
...
/* ═══════════════════════════════════════════════
   State: daftar file yang sudah dipilih
   (karena input[multiple] tidak bisa dimanipulasi
   langsung, kita track sendiri & rebuild DataTransfer)
═══════════════════════════════════════════════ */
let selectedFiles = []; // array of File objects

function syncInputFiles() {
    // Rebuild native FileList via DataTransfer
...
function handleImages(event) {
    ...
    // Tambah ke selectedFiles (hindari duplikat by name+size)
    valid.forEach(file => {
...
function renderPreviews() {
    ...
    countText.textContent     = `${selectedFiles.length} gambar dipilih`;
    ...
    selectedFiles.forEach((file, i) => {
        ...
        reader.onload = e => {
            ...
            item.innerHTML = `
                ...
                ${i === 0 ? '<span style="position:absolute;bottom:4px;left:4px;background:var(--secondary);color:white;font-size:9px;padding:2px 5px;border-radius:3px;">thumbnail</span>' : ''}
...
/* ═══════════════════════════════════════════════
   TinyMCE — Upload gambar ke editor (Promise-based, TinyMCE 6)
═══════════════════════════════════════════════ */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tinymce.init({
        ...
        // ✅ TinyMCE 6 — Promise-based handler
